...
export class AuthEffects {
        ...
        this.actions$.pipe(
            ...
            mergeMap(({ addresses }: { addresses: AddressPayload }) =>
                this.authService.saveUserAddressInService(addresses).pipe(
                    map((response) =>
                        AuthActions.addAddressSuccess({
                            addresses: response.data,
                        })
                    ),
                    catchError((error) =>
                        of(
                            AuthActions.addAddressFailure({
                                error: error.message,
                            })
                        )
                    )
        ...
        this.actions$.pipe(
            ...
            mergeMap(() =>
                this.authService.fetchUserAllAddressInService().pipe(
                    map((response) =>
                        AuthActions.fetchAddressesSuccess({
                            addresses: response.data,
                        })
                    ),
                    catchError((error) =>
                        of(
                            AuthActions.fetchAddressesFailure({
                                error: error.message,
                            })
                        )
                    )
    ...
    forgotEmailSend$ = createEffect(() =>
        this.actions$.pipe(
            ofType(AuthActions.ForgotEmailSendStart),
            switchMap(({ email }) =>
                this.authService.forgotPassword(email).pipe(
                    map((response: ApiResponse<null>) => {
                        if (response.success) {
                            return AuthActions.ForgotEmailSendSuccess({
                                response,
                            }); // ✅ Pass response correctly
                        } else {
                            return AuthActions.ForgotEmailSendFails({
                    ...
                    catchError((error) =>
                        of(
                            AuthActions.ForgotEmailSendFails({
                                error: error.message || 'An error occurred',
                            })
                        )
                    )
    ...
    verifyToken$ = createEffect(() =>
        this.actions$.pipe(
            ofType(AuthActions.verifyTokenStart),
            switchMap(({ token }) =>
                this.authService.verifyToken(token).pipe(
                    ...
                    catchError((error) =>
                        of(
                            AuthActions.verifyTokenFailure({
                                error: error.message,
                            })
                        )
                    )
