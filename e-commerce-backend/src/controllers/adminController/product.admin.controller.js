...
const { handleFileUpload, getSignedUrls } = require("../../utils/commonFunctions");
...
const createProduct = async (req, res) => {
    ...
    try {
        if (req.files.images) {
            const uploadedImages = await handleFileUpload(req.files.images, userId, 'images');
            images.push(...uploadedImages);
        }
        if (req.files.videos) {
            const uploadedVideos = await handleFileUpload(req.files.videos, userId, 'videos');
...
const deleteProduct = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Delete all images and videos from S3
        await Promise.all([...product.images, ...product.videos].map(async (s3FileName) => {
            try {
                await deleteFileFromS3(s3FileName);
            } catch (error) {
                console.error(`Error deleting file ${s3FileName} from S3:`, error);
                throw new Error(`Error deleting file ${s3FileName} from S3`);
            }
        }));
...
const uploadProductMedia = asyncHandler(async (req, res) => {
    ...
    try {
        const newImages = await handleFileUpload(req.files.images, userId, 'images');
        const newVideos = await handleFileUpload(req.files.videos, userId, 'videos');
        ...
        const signedImages = await Promise.all(product.images.map(async (s3FileName) => {
            const signedUrl = await getSignedAccessUrl(s3FileName);
            return signedUrl;
        }));

        const signedVideos = await Promise.all(product.videos.map(async (s3FileName) => {
            const signedUrl = await getSignedAccessUrl(s3FileName);
            return signedUrl;
        }));
        ...
        res.status(200).json(
            new ApiResponse(200, productWithSignedUrls, "Media uploaded successfully")
        );
    } catch (error) {
...
const deleteProductMedia = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await Promise.all(videoKeys.map(async (videoKey) => {
            try {
        ...
        product.videos = product.videos.filter(video => !deletedVideos.includes(video));
